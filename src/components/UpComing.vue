<template>
  <div class="bg-gradient-to-br min-h-[400px] from-[#05355d] to-[#032541] bg-opacity-60 mt-10 pt-4">
    <h1 class="flex items-center gap-2 border-b-4 border-[#2288db] pb-4 px-10 text-white font-bold mb-2">
      Up Coming Fun
      <PhFilmSlate :size="32" />
    </h1>
    <div class="flex items-center gap-6 overflow-x-auto px-10">
      ...
      <div v-else class="relative flex items-center shrink-0 w-[320px] text-center group transition-all mb-12 pt-5 text-white" v-for="trail in dataUpCom" key="trail.id">
        <button class="relative flex flex-col items-center" @click="modal(trail.id)">
          ...
          <p class="text-xl font-semibold">{{ trail.original_title }}</p>
        </button>
      </div>
    </div>
  </div>
